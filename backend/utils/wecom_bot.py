...
async def _generate_reply(content: str, wecom_user_id: str, chatid: str, reply_func=None, inline_data=None):
    ...
    try:
        ...
        if uid:
            ...
            if fit:
                context_str += (
                    ...
                    f"  (TSB说明: >5代表状态良好/巅峰；在-10到5代表状态平衡；在-30到-10代表黄金训练期，积极积累体能中；在-50到-30代表处于疲劳蓄积状态；小于-50代表严重疲劳/过度训练，必须安排减量或休息)\n"
        ...
        prompt_reqs = [
            ...
            "- 建议规则：如果用户想加量/加高强度，请严格检视他的 TSB（就绪度）和本周剩余课表。若 TSB < -50 代表严重疲劳，或在 -50 到 -30 之间代表明显疲劳，务必劝阻并建议休息；若 TSB 在 -30 到 -10 之间属于黄金训练区，支持训练但需注意身体反馈；遇到伤病咨询，优先建议恢复，不要鼓励硬顶。",
